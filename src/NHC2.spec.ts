...
describe('NHC2', () => {
    ...
    describe('sendStatusChangeCommand', () => {
        it('should send the status change command for device with value true', async (done) => {
            fakeMqttServer.server.on('published', function (packet, client) {
                if (packet.topic === STATUS_CHANGE_COMMAND_TOPIC) {
                    expect(packet.payload.toString()).toBe('{"Method":"devices.control","Params":[{"Devices":[{"Uuid":"488d61fa-de6c-4b1c-a832-f1971dc12110","Properties":[{"Status":"On"}]}]}]}');
                    done();
                }
            });
        ...
        it('should send the status change command for device with value true', async (done) => {
            fakeMqttServer.server.on('published', function (packet, client) {
                if (packet.topic === STATUS_CHANGE_COMMAND_TOPIC) {
                    expect(packet.payload.toString()).toBe('{"Method":"devices.control","Params":[{"Devices":[{"Uuid":"488d61fa-de6c-4b1c-a832-f1971dc12110","Properties":[{"Status":"Off"}]}]}]}');
                    done();
                }
            });
        ...
        it('should send the brightness change command for device with value 50', async (done) => {
            fakeMqttServer.server.on('published', function (packet, client) {
                if (packet.topic === BRIGHTNESS_CHANGE_COMMAND_TOPIC) {
                    expect(packet.payload.toString()).toBe('{"Method":"devices.control","Params":[{"Devices":[{"Uuid":"abd4b98b-f197-42ed-a51a-1681b9176228","Properties":[{"Brightness":"50"}]}]}]}');
                    done();
                }
            });
